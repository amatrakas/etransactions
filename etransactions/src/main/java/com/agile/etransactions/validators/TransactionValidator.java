...
public class TransactionValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if(requestDTO.getSourceAccountId() == null || requestDTO.getTargetAccountId() == null) {
            errors.reject(Constants.IBAN, "Null value(s) detected for account(s) IBAN");
            return;
        }
        ...
        if(requestDTO.getAmount() == null || requestDTO.getAmount().compareTo(BigDecimal.ZERO) == 0 || requestDTO.getAmount().compareTo(BigDecimal.ZERO) < 0) {
            errors.reject(Constants.AMOUNT, "Amount value should not be empty, zero or negative");
            return;
        }
        ...
        if(!sourceAccountExists && !targetAccountExists) {
            errors.reject(Constants.IBAN, "Source and target accounts does not exist");
            return;
        }else if(!sourceAccountExists) {
            errors.reject(Constants.IBAN, "Source account does not exist");
            return;
        }else if(!targetAccountExists){
            errors.reject(Constants.IBAN, "Target account does not exist");
            return;
        }

        // Prevent transaction between same accounts
        if(requestDTO.getSourceAccountId().equals(requestDTO.getTargetAccountId())) {
            errors.reject(Constants.ACCOUNT, "Transaction between same accounts is forbidden");
            return;
        }

        // Validate currency
        if(requestDTO.getCurrency() == null) {
            errors.reject(Constants.CURRENCY, "Currency type should not be empty");
            return;
        }

        if(!requestDTO.getCurrency().equals(CurrencyType.GBP.toString())) {
            errors.reject(Constants.CURRENCY, "Currency type should be 'GBP'");
            return;
        }
        ...
        try {
            ...
            if(balanceInsufficient) {
                errors.reject(Constants.BALANCE, "Transfer amount exceeds account balance");
            }
            logger.info("TransactionValidator.validate End");
        }catch (Exception e) {
            logger.error("Error in exchange API call: ", e);
            errors.reject(Constants.BALANCE, e.getMessage());
        }
    ...
    private boolean isBalanceInsufficient(String sourceAccountId, BigDecimal amount) throws IOException, InterruptedException {
        ...
        rateValue = CurrencyRateRetriever.getRateValue(account.getCurrency().toString());
